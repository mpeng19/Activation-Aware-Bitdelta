...
run_experiment() {
    ...
    echo "Running experiment: ${exp_name}_batch${batch_size}"
    ...
    EXP_DIR="$BASE_DIR/${exp_name}_batch${batch_size}"
    mkdir -p $EXP_DIR
    ...
    echo "Total time for ${exp_name}_batch${batch_size}: $total_time seconds"
...
for batch_size in 32; do
    echo "Running standard BitDelta with batch size $batch_size..."
    run_experiment "standard" false $batch_size
    
    echo "Running activation-aware BitDelta with batch size $batch_size..."
    run_experiment "activation_aware" true $batch_size
done
...
for batch_size in [1, 2, 4, 8, 16, 32]:
    for exp_name in ["standard", "activation_aware"]:
        exp_dir = f"{BASE_DIR}/{exp_name}_batch{batch_size}"
