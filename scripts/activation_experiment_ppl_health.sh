...
BASE_DIR="experiments/llama7b_calibration_agct"
mkdir -p $BASE_DIR
...
run_experiment() {
    ...
    python bitdelta/eval_ppl.py \
        --base_model "meta-llama/Llama-2-7b-hf" \
        --dataset_name "FremyCompany/AGCT-Dataset" \
        --subset "default" \
        --split "train" \
        --save_dir $EXP_DIR \
...
print("\nResults:")
print(df)
df.to_csv(f"{BASE_DIR}/medical_questions_results.csv", index=False)
EOF
}
...
echo "All experiments complete! Results saved to $BASE_DIR/medical_questions_results.csv"
